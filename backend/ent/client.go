...
// QueryUser queries the user edge of a Playlist_Video.
func (c *Playlist_VideoClient) QueryUser(pv *Playlist_Video) *UserQuery {
	query := &UserQuery{config: c.config}
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := pv.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(playlist_video.Table, playlist_video.FieldID, id),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, playlist_video.UserTable, playlist_video.UserColumn),
		)
		fromV = sqlgraph.Neighbors(pv.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryPlaylistVideos queries the playlist_videos edge of a User.
func (c *UserClient) QueryPlaylistVideos(u *User) *PlaylistVideoQuery {
	query := &PlaylistVideoQuery{config: c.config}
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(playlist_video.Table, playlist_video.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.PlaylistVideosTable, user.PlaylistVideosColumn),
		)
		fromV = sqlgraph.Neighbors(u.driver.Dialect(), step)
		return fromV, nil
	}
