...
// was not loaded in eager-loading, or loaded but was not found.
func (e Playlist_VideoEdges) UserOrErr() (*User, error) {
	if e.loadedTypes[3] {
		if e.User == nil {
...
// QueryUser queries the user edge of the Playlist_Video.
func (pv *Playlist_Video) QueryUser() *UserQuery {
	return (&Playlist_VideoClient{config: pv.config}).QueryUser(pv)
}
