...
func (uq *UserQuery) QueryPlaylistVideos() *PlaylistVideoQuery {
	query := &PlaylistVideoQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithPlaylistVideos(opts ...func(*PlaylistVideoQuery)) *UserQuery {
	query := &PlaylistVideoQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withPlaylistVideos = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withPlaylistVideos; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Playlist_Video(func(s *sql.Selector) {
			s.Where(sql.InValues(user.PlaylistVideosColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
